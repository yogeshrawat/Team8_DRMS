...
public class RMIServer extends Thread implements StudentInterface,
		AdminInterface {
	...
	public static void main(String[] args) throws RemoteException,
		...
		Remote objremote1 = UnicastRemoteObject.exportObject(Server1,
				rmiRegistryPort);
		rmiRegistry.bind("Concordia", objremote1);

		Remote objremote2 = UnicastRemoteObject.exportObject(Server2,
				rmiRegistryPort);
		rmiRegistry.bind("Ottawa", objremote2);

		Remote objremote3 = UnicastRemoteObject.exportObject(Server3,
				rmiRegistryPort);
	...
	public static void addData(RMIServer server) throws RemoteException {
		...
		server.createAccount("Student" + i, "L" + i, "Student" + i
				+ "@test.com", "123456", "" + i, "abcd", server.instituteName);
		i++;
	}
	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				String response = GetNonReturnersByServer(Integer.parseInt(data
						.trim()));
	...
	public boolean createAccount(String strFirstName, String strLastName,
		...
		synchronized (tableStudents) {
			ArrayList<Student> objNewStudent = tableStudents.get(strUsername
					.charAt(0));
	...
	public boolean reserveBook(String strUsername, String strPassword,
			String strBookName, String strAuthor) throws RemoteException {
	...
	public int checkUser(String strUsername, String strPassword,
			String strEducationalInstitution) throws RemoteException {
		...
		if (listStudent.size() > 0) {
			for (Student student : listStudent) {
				if (student.getUserName().equals(strUsername)&&student.getPassword().equals(strPassword)) {
						login = 1;
				}
			}
		}
		else{
			System.out.println("Couldnt load Data");
		}
				...
				synchronized (Server) {
					if (!Server.instituteName.equals(this.instituteName)) {
						...
						try {
							...
							DatagramPacket reply = new DatagramPacket(msgIn,
									msgIn.length);
